...
func parser(dbName string) {
	dsn := flag.String("dsn", fmt.Sprintf("root:123456@tcp(192.168.0.64:3306)/%s?charset=utf8", dbName), "数据库dsn配置")
	file := flag.String("file", fmt.Sprintf("../kxmj.common/entities/%s/", dbName), "保存路径")
	table := flag.String("table", "", "要迁移的表")
	realNameMethod := flag.String("realNameMethod", "TableName", "结构体对应的表名")
	packageName := flag.String("packageName", dbName, "生成的struct包名")
