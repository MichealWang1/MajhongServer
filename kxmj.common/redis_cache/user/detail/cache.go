...
func (c *Cache) Get(ctx context.Context, userId uint32) (*kxmj_core.User, error) {
	key := generateKey(userId)
	value := &kxmj_core.User{}
...
func (c *Cache) Set(ctx context.Context, user *kxmj_core.User) error {
...
func generateKey(userId uint32) string {
	return fmt.Sprintf(keys.UserInfoFormatKey, userId)
}
