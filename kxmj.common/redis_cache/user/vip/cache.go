...
func (c *Cache) EventHandler(ctx context.Context, e *redis_core.EventParams) {
	...
	params := e.Data.(*kxmj_core.UserVip)
	if params.UserId <= 0 {
		return
	}
...
func (c *Cache) Get(ctx context.Context, userId uint32) (*kxmj_core.UserVip, error) {
	key := generateKey(userId)
	value := &kxmj_core.UserVip{}
...
func (c *Cache) Set(ctx context.Context, user *kxmj_core.UserVip) error {
...
func generateKey(userId uint32) string {
	return fmt.Sprintf(keys.UserVIPFormatKey, userId)
}
