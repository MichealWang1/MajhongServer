...
// GetSystemAllMail 获取玩家系统邮件
func (c *Cache) GetSystemAllMail(ctx context.Context, userId uint32) (map[uint32]uint8, error) {
...
// SetSystemMail 设置玩家单个邮件的状态 HSet
func (c *Cache) SetSystemMail(ctx context.Context, userId uint32, mailId uint32, state uint8) error {
	key := generateSystemMailKey(userId)
...
// GetSystemMail 获取玩家指定 系统邮件的状态 HGet
func (c *Cache) GetSystemMail(ctx context.Context, userId uint32, mailId uint32) (uint8, error) {
	key := generateSystemMailKey(userId)
	field := fmt.Sprintf("%d", mailId)
...
// DelUserSystemMail 删除玩家所有的 福利邮件
func (c *Cache) DelUserSystemMail(ctx context.Context, userId uint32) error {
	key := generateSystemMailKey(userId)
...
// DelUserSystemMailByMailId 删除玩家单封福利邮件
func (c *Cache) DelUserSystemMailByMailId(ctx context.Context, userId uint32, mailId uint32) error {
	key := generateSystemMailKey(userId)
	field := fmt.Sprintf("%d", mailId)
