...
// Get 获取玩家指定 福利邮件的状态 HGet
func (c *Cache) Get(ctx context.Context, userId uint32, mailId uint32) (*EmailUser, error) {
...
// GetAll 获取玩家所有的福利邮件
func (c *Cache) GetAll(ctx context.Context, userId uint32) (map[uint32]*EmailUser, error) {
	key := generateWelfareMailKey(userId)
...
// Set 设置玩家单个福利邮件的状态 HMSet
func (c *Cache) Set(ctx context.Context, userId uint32, mailId uint32, data *EmailUser) error {
	key := generateWelfareMailKey(userId)
	field := fmt.Sprintf("%d", mailId)
