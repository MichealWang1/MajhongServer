...
func (rs *RpcxServer) Start() {
	for _, cfg := range rs.servicesCfg {
		...
		} else if cfg.Type == service.PaymentService {
			rs.payment = client.NewXClient(servicePath, client.Failtry, client.RandomSelect, d, client.DefaultOption)
		}
	}
}
...
func (rs *RpcxServer) PaymentClient() client.XClient {
	return rs.payment
}
